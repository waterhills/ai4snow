    ...
    <nav className="fixed top-0 w-full z-50 bg-[#010e24]/40 backdrop-blur-xl border-b border-[#72dcff]/15 shadow-[0_0_20px_rgba(114,220,255,0.05)]">
      <div className="flex justify-between items-center px-8 h-20 w-full max-w-screen-2xl mx-auto font-headline tracking-tight">
        ...
        <div className="hidden md:flex gap-10 items-center font-headline">
          <Link className={navLinkStyle('/')} to="/">首页</Link>
          <Link className={navLinkStyle('/upload')} to="/upload">上传</Link>
          <Link className={navLinkStyle('/results')} to="/results">结果</Link>
          <Link className={navLinkStyle('/pricing')} to="/pricing">方案</Link>
        </div>
        ...
        <div className="flex items-center gap-4">
          {isLoggedIn ? (
            <div className="flex gap-4 items-center">
              <div className="hidden md:flex items-center px-3 py-1 bg-surface-container-high rounded-full border border-primary/20">
                <span className="text-[10px] text-on-surface-variant font-bold uppercase tracking-widest mr-2">代币库</span>
          ...
          {isLoggedIn && (
            <Link to="/pricing" title="个人控制面板" className="material-symbols-outlined text-[#72dcff] hover:bg-[#72dcff]/10 hover:shadow-[0_0_15px_rgba(114,220,255,0.3)] transition-all duration-300 p-2 rounded-full active:scale-95">
              account_circle
            </Link>
          )}
        </div>
